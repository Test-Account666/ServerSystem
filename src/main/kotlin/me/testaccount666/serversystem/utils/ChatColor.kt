...
enum class ChatColor {
     ...
     */
    val chatColor = ChatColor.valueOf(name)
    ...
    companion object {
         ...
         */
        private val _STRIP_COLOR_PATTERN = "(?i)$COLOR_CHAR[0-9A-FK-OR]".toRegex()

        /**
         * Logger for error reporting.
         */
        private val _LOGGER = Logger.getLogger(ChatColor::class.java.name)
        ...
        fun translateAlternateColorCodes(altColorChar: Char, text: String?): String {
            ...
            val processedText = when {
                text.contains("$altColorChar#") -> {
                    runCatching { replaceHexColor(altColorChar, text) }
                        .onFailure { _LOGGER.log(Level.WARNING, "Error processing hex colors", it) }
                        .getOrNull() ?: text
                }
            ...
            return ChatColor.translateAlternateColorCodes(altColorChar, processedText)
        }
        ...
        fun replaceHexColor(altColorChar: Char, input: String?): String {
            ...
            val regex = HEX_PATTERN.pattern.replace("&", Regex.escape(altColorChar.toString())).toRegex()
            return regex.replace(input) { convertHexToMinecraft(it.groupValues[2]) }
        }
        ...
        fun stripColor(input: String?): String {
            if (input.isNullOrEmpty()) return ""
            return _STRIP_COLOR_PATTERN.replace(input, "")
        }
    }
}
