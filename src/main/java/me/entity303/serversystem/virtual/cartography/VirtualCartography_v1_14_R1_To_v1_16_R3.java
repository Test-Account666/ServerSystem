...
public class VirtualCartography_v1_14_R1_To_v1_16_R3 extends VirtualCartography {
    ...
    private Constructor containerCartographyConstructor = null;
    ...
    public void openCartography(Player player) {
        ...
        if (this.containerCartographyConstructor == null) try {
            this.containerCartographyConstructor = Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerCartography").getDeclaredConstructor(int.class, Class.forName("net.minecraft.server." + this.getVersion() + ".PlayerInventory"), Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerAccess"));
        } catch (NoSuchMethodException | ClassNotFoundException e) {
            e.printStackTrace();
            return;
        }
        ...
        Object containerCartography;

        try {
            containerCartography = this.containerCartographyConstructor.newInstance(windowID, playerInventory, this.getWrapper(player));
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            this.checkReachableField.set(containerCartography, false);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            component = this.aMethod.invoke(null, "{\"text\":\"Cartography\"}");
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            packetPlayOutOpenWindow = this.packetPlayOutOpenWindowConstructor.newInstance(windowID, Class.forName("net.minecraft.server." + this.getVersion() + ".Containers").getDeclaredField("CARTOGRAPHY_TABLE").get(null), component);
        ...
        try {
            this.activeContainerField.set(entityPlayer, containerCartography);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
