...
public class VirtualCartography_Latest extends AbstractVirtualCartography {
    ...
    public void OpenCartography(Player player) {
        ...
        ContainerCartography container;
        try {
            container = new ContainerCartography(containerId, (PlayerInventory) AbstractVirtual.GET_INVENTORY_METHOD.invoke(human), (ContainerAccess) this.GetWrapper(player));
        ...
        try {
            ...
            AbstractVirtual.SEND_PACKET_METHOD.invoke(playerConnection,
                                                      new PacketPlayOutOpenWindow(containerId, Containers.r, IChatBaseComponent.ChatSerializer.a("{\"text\":\"Cartography\"}")));
        } catch (IllegalAccessException | InvocationTargetException exception) {
            exception.printStackTrace();
            return;
        }
