...
public class VirtualLoom_v1_14_R1_To_v1_16_R3 extends VirtualLoom {
    ...
    private Constructor containerLoomConstructor = null;
    ...
    public void openLoom(Player player) {
        ...
        if (this.containerLoomConstructor == null) try {
            this.containerLoomConstructor = Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerLoom").getDeclaredConstructor(int.class, Class.forName("net.minecraft.server." + this.getVersion() + ".PlayerInventory"), Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerAccess"));
        } catch (NoSuchMethodException | ClassNotFoundException e) {
            e.printStackTrace();
            return;
        }
        ...
        Object containerLoom;

        try {
            containerLoom = this.containerLoomConstructor.newInstance(windowID, playerInventory, this.getWrapper(player));
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            this.checkReachableField.set(containerLoom, false);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            component = this.aMethod.invoke(null, "{\"text\":\"Loom\"}");
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            packetPlayOutOpenWindow = this.packetPlayOutOpenWindowConstructor.newInstance(windowID, Class.forName("net.minecraft.server." + this.getVersion() + ".Containers").getDeclaredField("LOOM").get(null), component);
        ...
        try {
            this.activeContainerField.set(entityPlayer, containerLoom);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
