...
public class VirtualStoneCutter_Latest extends AbstractVirtualStoneCutter {
    ...
    public void OpenStoneCutter(Player player) {
        ...
        try {
            container = new ContainerStonecutter(containerId, (PlayerInventory) AbstractVirtual.GET_INVENTORY_METHOD.invoke(human), (ContainerAccess) this.GetWrapper(player));
        ...
        try {
            ...
            AbstractVirtual.SEND_PACKET_METHOD.invoke(playerConnection,
                                                      new PacketPlayOutOpenWindow(containerId, Containers.x, IChatBaseComponent.ChatSerializer.a("{\"text\":\"Stonecutter\"}")));
        } catch (IllegalAccessException | InvocationTargetException exception) {
            exception.printStackTrace();
            return;
        }
