...
public class SignListener implements Listener {
    ...
    public void OnSignColorCode(SignChangeEvent event) {
        if (event.getLine(0).equalsIgnoreCase("[warp]")) {
            if (!this._plugin.GetPermissions().HasPermission(event.getPlayer(), "signs.warp", true)) return;
        ...
        if (event.getLine(0).equalsIgnoreCase("[disposal]")) {
            if (!this._plugin.GetPermissions().HasPermission(event.getPlayer(), "signs.disposal", true)) return;
            event.setLine(0, ChatColor.GRAY + "[" + ChatColor.DARK_AQUA + "Disposal" + ChatColor.GRAY + "]");
            return;
        }
        if (!this._plugin.GetPermissions().HasPermission(event.getPlayer(), "signs.color", true)) return;
