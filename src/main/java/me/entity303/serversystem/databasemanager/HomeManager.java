...
public class HomeManager {
    ...
    public Map<String, Location> GetHomes(OfflinePlayer player) {
        ...
        if (!homeFile.exists()) return new HashMap<>();

        if (homeFile.exists()) {
            Collection<String> homes = new ArrayList<>(homeCfg.getConfigurationSection("Homes").getKeys(false));
            if (homes.isEmpty()) return new HashMap<>();

            return homes.stream()
                        .collect(Collectors.toMap(home -> home, home -> (Location) homeCfg.get("Homes." + home.toUpperCase()), (location, location1) -> location1));
        }
        return new HashMap<>();
    }
    ...
    public HomeManager DeleteHome(OfflinePlayer player, String home) {
        ...
        if (!homeFile.exists()) return this;

        if (homeFile.exists()) {
            List<String> homes = new ArrayList<>(homeCfg.getConfigurationSection("Homes").getKeys(false));
            if (homes.isEmpty()) return this;
        }
