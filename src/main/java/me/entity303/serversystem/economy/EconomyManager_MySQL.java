...
public class EconomyManager_MySQL extends AbstractEconomyManager {
    ...
    public Double GetMoneyAsNumber(OfflinePlayer offlinePlayer) {
        ...
        if (Bukkit.isPrimaryThread()) if (this._moneyCache.containsKey(offlinePlayer)) return this._moneyCache.get(offlinePlayer);
        var resultSet =
                this.GetPlugin().GetMySQL().GetResult("SELECT * FROM Economy WHERE UUID = '" + offlinePlayer.getUniqueId() + "' AND Server = '" + this._server + "'");
        try {
            try {
    ...
    public void DeleteAccount(OfflinePlayer offlinePlayer) {
        ...
        if (offlinePlayer == null) return;
        this.GetPlugin().GetMySQL().ExecuteUpdate("DELETE FROM Economy WHERE UUID='" + offlinePlayer.getUniqueId() + "'" + " AND Server = '" + this._server + "'");
    ...
    public boolean HasAccount(OfflinePlayer offlinePlayer) {
        if (offlinePlayer == null) return false;
        if (Bukkit.isPrimaryThread()) if (this._accountCache.containsKey(offlinePlayer)) return this._accountCache.get(offlinePlayer);
        ResultSet resultSet;
        resultSet =
                this._plugin.GetMySQL().GetResult("SELECT * FROM Economy WHERE UUID = '" + offlinePlayer.getUniqueId() + "'" + " AND Server = '" + this._server + "'");
        try {
            ...
            if (!throwables.getMessage().toLowerCase().contains("closed")) throwables.printStackTrace();
        }
        return false;
    }
    ...
    public LinkedHashMap<OfflinePlayer, Double> GetTopTen() {
        ...
        try {
            resultSet = this._plugin.GetMySQL().GetResult("SELECT * " + "FROM Economy " + "WHERE Server='" + this._server + "' " + "ORDER BY Balance desc " + "LIMIT 10");
