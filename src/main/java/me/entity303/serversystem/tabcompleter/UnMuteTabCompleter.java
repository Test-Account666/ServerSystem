...
public class UnMuteTabCompleter implements ITabCompleterOverload {
    ...
    public List<String> onTabComplete(CommandSender commandSender, Command command, String commandLabel, String[] arguments) {
        if (!this._plugin.GetPermissions().HasPermission(commandSender, "unmute", true)) return Collections.singletonList("");
        if (arguments.length != 1) return Collections.singletonList("");

        if (this._plugin.GetMuteManager().GetMutedPlayerNames().isEmpty()) return Collections.singletonList("");
