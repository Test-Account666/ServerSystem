...
public class MuteManager_SQLite extends ManagerMute {
    ...
    public MuteManager_SQLite(ServerSystem plugin, String dateFormat) {
        ...
        try {
            this.connection.createStatement()
                           .executeUpdate(
                                   "CREATE TABLE IF NOT EXISTS MutedPlayers (BannedUUID VARCHAR(100), SenderUUID VARCHAR(100), Reason VARCHAR(100), Shadow INT, UnbanTime BIGINT)");
    ...
    public boolean open() {
        if (this.initialize())
            try {
                this.connection = DriverManager.getConnection("jdbc:sqlite:" + new File("plugins//ServerSystem", "mutes.sqlite").getAbsolutePath());
                return true;
