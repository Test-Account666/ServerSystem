        ...
        Bukkit.getScheduler().runTaskAsynchronously(this._plugin, () -> {
            ...
            if (!REPLY_MAP.containsKey(commandSender)) {
                ...
                commandSender.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                          this._plugin.GetMessages().GetMessage(commandLabel, command, commandSender, null, "Reply"));
                return;
            }
                    ...
                    if (!unmuted) {
                        if (!mute.IsShadow()) {
                            ...
                            player.sendMessage(this._plugin.GetMessages().GetPrefix() + this._plugin.GetMessages()
                                                                                                    .GetMessage("mute", "mute", senderName, player,
                                                                                                                "Mute.Muted")
                        ...
                        commandSender.sendMessage(this._plugin.GetMessages()
                                                              .GetMessage(commandLabel, command, commandSender, target, "Msg.Sender")
                                                              .replace("<MESSAGE>", msg));
                        return;
                    }
                }
            ...
            target.sendMessage(
                    this._plugin.GetMessages().GetMessage(commandLabel, command2, commandSender, target, "Msg.Target").replace("<MESSAGE>", msg));
            var command1 = command.getName();
            commandSender.sendMessage(
                    this._plugin.GetMessages().GetMessage(commandLabel, command1, commandSender, target, "Msg.Sender").replace("<MESSAGE>", msg));

            for (var all : this._plugin.GetSocialSpy())
                all.sendMessage(
                        this._plugin.GetMessages().GetMessage(commandLabel, command, commandSender, target, "Msg.SocialSpy").replace("<MESSAGE>", msg));
