...
@ServerSystemCommand(name = "TeleportRequestHere")
public class TeleportRequestHereCommand implements ICommandExecutorOverload {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String commandLabel, String[] arguments) {
        ...
        if (arguments.length == 0) {
            commandSender.sendMessage(
                    this._plugin.GetMessages().GetPrefix() + this._plugin.GetMessages().GetSyntax(commandLabel, command.getName(), commandSender, null, "Tpahere"));
            return true;
        }
        ...
        commandSender.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                  this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "TpaHere.Sender"));
        targetPlayer.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                 this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "TpaHere.Target"));

        var accept = new TextComponent(
                this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "TpaHere.Accept", true).replace("&", "§"));
        ...
        var deny = new TextComponent(
                this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "TpaHere.Deny", true).replace("&", "§"));
