...
@ServerSystemCommand(name = "TeleportRequest")
public class TeleportRequestCommand implements ICommandExecutorOverload {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String commandLabel, String[] arguments) {
        if (this._plugin.GetPermissions().GetConfiguration().GetBoolean("Permissions.tpa.required")) {
            if (!this._plugin.GetPermissions().HasPermission(commandSender, "tpa.permission")) {
                commandSender.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                          this._plugin.GetMessages().GetNoPermission(this._plugin.GetPermissions().GetPermission("tpa.permission")));
                return true;
            }
        }
        ...
        if (arguments.length == 0) {
            commandSender.sendMessage(
                    this._plugin.GetMessages().GetPrefix() + this._plugin.GetMessages().GetSyntax(commandLabel, command.getName(), commandSender, null, "Tpa"));
            return true;
        }
        ...
        if (this._plugin.GetTpaDataMap().containsKey(targetPlayer)) {
            ...
            if (tpaData.GetEnd() < System.currentTimeMillis()) {
                commandSender.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                          this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "Tpa.PendingTpa"));
                return true;
            } else {
        ...
        commandSender.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                  this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "Tpa.Sender"));
        targetPlayer.sendMessage(this._plugin.GetMessages().GetPrefix() +
                                 this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "Tpa.Target"));

        var accept = new TextComponent(
                this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "Tpa.Accept", true).replace("&", "§"));
        ...
        var deny = new TextComponent(
                this._plugin.GetMessages().GetMessage(commandLabel, command.getName(), commandSender, targetPlayer, "Tpa.Deny", true).replace("&", "§"));
