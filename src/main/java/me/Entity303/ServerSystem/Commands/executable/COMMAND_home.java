...
public class COMMAND_home implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        if (this.plugin.getConfig().getBoolean("teleportation.home.enabledelay") && !this.plugin.getPermissions().hasPerm(cs, "home.bypassdelay", true)) {
            this.plugin.getTeleportMap().put(((Player) cs), Bukkit.getScheduler().runTaskLater(this.plugin, () -> {
                ...
                if (player.isOnline()) {
                    ...
                    cs.sendMessage(COMMAND_home.this.plugin.getMessages().getPrefix() + ChatColor.translateAlternateColorCodes('&', COMMAND_home.this.plugin.getMessages().getCfg().getString("Messages.Misc.Teleportation.Success")));
                    COMMAND_home.this.plugin.getTeleportMap().remove(player);
                }
            }, 20L * this.plugin.getConfig().getInt("teleportation.home.delay")));
