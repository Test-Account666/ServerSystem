...
public class VirtualStoneCutter_v1_14_R1_To_v1_16_R3 extends VirtualStoneCutter {
    ...
    private Constructor containerStonecutterConstructor = null;
    ...
    public void openCutter(Player player) {
        ...
        if (this.containerStonecutterConstructor == null) try {
            this.containerStonecutterConstructor = Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerStonecutter").getDeclaredConstructor(int.class, Class.forName("net.minecraft.server." + this.getVersion() + ".PlayerInventory"), Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerAccess"));
        } catch (NoSuchMethodException | ClassNotFoundException e) {
            e.printStackTrace();
            return;
        }
        ...
        Object containerStonecutter;

        try {
            containerStonecutter = this.containerStonecutterConstructor.newInstance(windowID, playerInventory, this.getWrapper(player));
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            this.checkReachableField.set(containerStonecutter, false);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            component = this.aMethod.invoke(null, "{\"text\":\"Stonecutter\"}");
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            packetPlayOutOpenWindow = this.packetPlayOutOpenWindowConstructor.newInstance(windowID, Class.forName("net.minecraft.server." + this.getVersion() + ".Containers").getDeclaredField("STONECUTTER").get(null), component);
        ...
        try {
            this.activeContainerField.set(entityPlayer, containerStonecutter);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
