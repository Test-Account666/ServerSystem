...
public class VirtualSmithing_v1_16_R1_To_v1_16_R3 extends VirtualSmithing {
    ...
    private Constructor containerSmithingConstructor = null;
    ...
    public void openSmithing(Player player) {
        ...
        if (this.containerSmithingConstructor == null) try {
            this.containerSmithingConstructor = Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerSmithing").getDeclaredConstructor(int.class, Class.forName("net.minecraft.server." + this.getVersion() + ".PlayerInventory"), Class.forName("net.minecraft.server." + this.getVersion() + ".ContainerAccess"));
        } catch (NoSuchMethodException | ClassNotFoundException e) {
            e.printStackTrace();
            return;
        }
        ...
        Object containerSmithing;

        try {
            containerSmithing = this.containerSmithingConstructor.newInstance(windowID, playerInventory, this.getWrapper(player));
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            this.checkReachableField.set(containerSmithing, false);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            component = this.aMethod.invoke(null, "{\"text\":\"Smithing\"}");
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
            return;
        }
        ...
        try {
            packetPlayOutOpenWindow = this.packetPlayOutOpenWindowConstructor.newInstance(windowID, Class.forName("net.minecraft.server." + this.getVersion() + ".Containers").getDeclaredField("SMITHING").get(null), component);
        ...
        try {
            this.activeContainerField.set(entityPlayer, containerSmithing);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return;
        }
