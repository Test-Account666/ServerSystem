...
public class VaultHookManager {
    ...
    public VaultHookManager(ss plugin) {
        this.plugin = plugin;
    }

    public void hook() {
        if (!this.plugin.getConfig().getBoolean("economy.enabled")) return;
    ...
    public void unhook() {
        ...
        if (!this.plugin.getConfig().getBoolean("economy.hookintovault")) return;
        ServicesManager sm = this.plugin.getServer().getServicesManager();
        if (this.economy != null) {
